...
const MCP_TOOLS: McpTool[] = [
  ...
  subcommandTool(
    "imagent_video",
    "video",
    'Submit, track, and download video jobs. First run `imagent_models --kind video` and `imagent_options --provider <id> --model <id> --kind video` to learn which models are available and which `--option key=value` keys they accept (durationSec, resolution, aspectRatio, fps, firstFrame, ...). `generate` defaults to async submission; pass `--wait` to poll until completion and download into the gallery, plus `--out <dir>` to copy the downloaded file. Use `task ls`, `task get --id <jobId>`, and `task cancel --id <jobId>` for async jobs; running tasks refresh remote provider status before returning. Download with `download --id <jobId>`. Args go after `imagent video`, e.g. ["generate", "a camera orbit around a glass sculpture", "--provider", "google", "--model", "veo-3.0-generate-001", "--option", "durationSec=8", "--wait", "--out", "./outputs"].',
  ),
  subcommandTool(
    "imagent_config",
    "config",
    'Inspect, edit, or reset local provider credentials, preferences, and per-user routing. Subcommands: `set <key> <value>`, `get [key]`, `path` (locate config.json/catalog.json/secrets.json), `reset <target>` where target is `catalog` | `secrets` | `config`, and `provider <add|rm|list>` for per-user model routing (Azure deployments, custom OpenAI providers). Recognised secret keys: <vendor>.apiKey, azure.endpoint, byteplus.endpoint, volcengine.endpoint, <vendor>.baseUrl. Vendors: openai | azure | google | flux-bfl | byteplus | volcengine | xai | minimax. Args go after `imagent config`, e.g. ["set", "openai.apiKey", "sk-..."], ["provider", "add", "azure", "my-deployment", "--model", "gpt-image-2"], ["provider", "list"], or ["reset", "catalog", "--force"]. Use `imagent_models` / `imagent_options` to inspect what canonical model ids exist.',
  ),
  subcommandTool(
    "imagent_asset",
    "asset",
