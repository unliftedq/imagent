...
const MCP_TOOLS: McpTool[] = [
  ...
  subcommandTool(
    "imagent_config",
    "config",
    'Inspect, edit, or reset local provider credentials, preferences, and per-user routing. Subcommands: `set <key> <value>`, `get [key]`, `path` (locate config.json/catalog.json/secrets.json), `reset <target>` where target is `catalog` | `secrets` | `config`, and `provider <add|rm|list>` for per-user model routing (Azure deployments, custom OpenAI providers). Recognised secret keys: <vendor>.apiKey, azure.endpoint, byteplus.endpoint, volcengine.endpoint, <vendor>.baseUrl. Vendors: openai | azure | google | flux-bfl | byteplus | volcengine | xai. Args go after `imagent config`, e.g. ["set", "openai.apiKey", "sk-..."], ["provider", "add", "azure", "my-deployment", "--model", "gpt-image-2"], ["provider", "list"], or ["reset", "catalog", "--force"]. Use `imagent_models` / `imagent_options` to inspect what canonical model ids exist.',
  ),
  subcommandTool(
    "imagent_asset",
    "asset",
