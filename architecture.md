...
The core domain separates image and video generation because their lifecycles differ:
...
The job runner coordinates generation across both kinds. It persists state transitions to SQLite, emits progress events, stores completed files, and creates gallery records. Persisted queued/running jobs allow long-running video work to be inspected or resumed by later desktop or CLI sessions.

## Providers and Model Catalog

Providers are configured at the provider/vendor level, while models are resolved from the catalog. The catalog has canonical image/video model definitions and provider-specific offerings. An offering maps the provider-facing model or deployment ID to a canonical model and may override capabilities or defaults.
