...
| Providers v1 | All six day-one: OpenAI, Azure OpenAI, Google (Imagen/Gemini), Flux (BFL official `api.bfl.ai`), Seedream, Seedance. |
...
**Deliverables**
- `packages/providers/src/{openai,azure,google,flux,volcengine}/{image,video}.ts`. Volcengine has both ports; OpenAI/Azure/Google/Flux are image-only at v1.
...
- `imagine video <prompt> --provider seedance [--wait]` (without `--wait`, prints job id and exits; with `--wait`, polls and prints progress).
...
**Acceptance**
- `imagine video "rotating crystal in a misty forest" --provider seedance --wait` blocks, prints progress, then prints the MP4 path.
...
| M3 | `imagine video "rotating crystal" --provider seedance --wait` | MP4 path printed |
